...
//go:generate swagger generate server --target ../../gen --name Example --spec ../../swagger/swagger.yml --principal interface{} --exclude-main
...
func configureAPI(api *operations.ExampleAPI) http.Handler {
	...
	if api.SessionSecurityAuth == nil {
		api.SessionSecurityAuth = func(token string) (interface{}, error) {
			return nil, errors.NotImplemented("api key auth (SessionSecurity) Cookie from header param [Cookie] has not yet been implemented")
		}
	}
	...
	if api.UserMyselfHandler == nil {
		api.UserMyselfHandler = user.MyselfHandlerFunc(func(params user.MyselfParams, principal interface{}) middleware.Responder {
			return middleware.NotImplemented("operation user.Myself has not yet been implemented")
		})
	}
